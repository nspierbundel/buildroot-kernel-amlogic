...
inline void vdin_get_format_convert(struct vdin_dev_s *devp)
{
	if (devp->prop.color_format == TVIN_RGB444)
	{
        ...
        else if(devp->prop.color_format == TVIN_NV21)
...
static inline void vdin_set_top(unsigned int offset, enum tvin_port_e port, enum tvin_color_fmt_e cfmt,unsigned int h)
{
	//    unsigned int offset = devp->addr_offset;
...
static inline void vdin_set_wr_ctrl(unsigned int offset, unsigned int v, unsigned int h, enum vdin_format_convert_e format_convert)
{
	...
	WR_BITS(VDIN_WR_V_START_END, (v -1), WR_VEND_BIT, WR_VEND_WID);
        ...
        // swap_cbcr
        WR_BITS(VDIN_WR_CTRL,
                (format_convert == VDIN_FORMAT_CONVERT_YUV_NV21)?1:0, SWAP_CBCR_BIT, SWAP_CBCR_WID);
        //chroma canvas
...
void set_wr_ctrl(int h_pos,int v_pos,struct vdin_dev_s *devp)
{
	...
	if(v_pos + fmt_info->vs_bp<0)
	{
		WR_BITS(VDIN_WR_V_START_END,
				abs(v_pos + fmt_info->vs_bp), WR_VSTART_BIT, WR_VSTART_WID);
	}
 ...
 */
inline void vdin_set_hscale(unsigned int offset, int src_w,  int dst_w)
{
